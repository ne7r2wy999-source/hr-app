...
def analyze_doc_with_ai(image_obj, api_key):
    """تحليل الوثيقة مع تجربة الموديلات المتاحة تلقائياً تجنباً لأخطاء 404"""
    genai.configure(api_key=api_key)
    
    # قائمة الموديلات التجريبية بالترتيب لضمان النجاح
    candidate_models = ['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-1.0-pro-vision-latest', 'gemini-pro-vision']
    ...
    # لو كل الموديلات المحددة فشلت، بنجرب نجيب الموديل المتاح في حسابك تلقائياً
    try:
        for m in genai.list_models():
            if 'generateContent' in m.supported_generation_methods and 'vision' in m.name or 'flash' in m.name:
    ...
    raise RuntimeError(f"تعذر الاتصال بموديل Gemini، الخطأ: {last_error}")
